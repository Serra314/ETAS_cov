...
# triggering function.
g.t.toplot <- function(theta, eta, tt, th, M0){
  # initialize output
  output <- rep(0, length(th))
  t.diff <- tt - th
  upd <- t.diff > 0
  if(sum(upd) > 0){
    output[upd] <- theta[2]*exp(eta)*( (1 + t.diff[upd]/theta[4])^(-theta[5]) )
    output  
  }
  else{
    output
...
# triggering function.
g.x <- function(theta, eta, tt, xx, yy, th, xh, yh, mh, M0, Sigma){
  # initialize output
  output <- rep(0, length(th))
  t.diff <- tt - th
  upd <- t.diff > 0
  if(sum(upd) > 0){
    output[upd] <- theta[2]*exp(eta)*( (1 + t.diff[upd]/theta[4])^(-theta[5]) )*g.s(xx,yy,xh[upd],yh[upd],Sigma)
    output  
  }
  else{
    output
...
lambda <- function(theta, eta, tt, xx, yy, th, xh, yh, mh, M0, Sigma){
  theta[1]  + sum(g.x(theta, eta, tt, xx, yy, th, xh, yh, mh, M0, Sigma))
}
...
logLambda.h.vec <- function(theta, eta, th, xh, yh, mh, M0, T1, T2, bdy, Sigma){
  ...
  # integral in time
  Tl <- pmax(th, as.numeric(T1))
  
  gamma.T2 <- ( 1 + (T2 - th)/theta[4] )^(1-theta[5])  
  gamma.Tl <- ( 1 + (Tl - th)/theta[4] )^(1-theta[5])
  
  # output
  log(theta[2]) + eta + log(theta[4]) - log(theta[5] - 1) + log(gamma.Tl - gamma.T2) + log(I.s)
}
...
ETAS.fit.cov <- function(sample.s, 
                               M0, T1, T2, bdy,
  ...
  logLambda.h.inla <- function(th.K, th.alpha, th.c, th.p, th.sigma, 
                               th.beta.depth = NULL,
                               th.beta.strike = NULL,
                               list.input, model.type = 'basic',
                               M0, bdy, ncore_ = ncore){
    theta_ <- c(0, 
    ...
    if(model.type == 'basic'){
      eta = theta_[3]*(list.input$df_grid$mags - M0)
      #cat('mean eta:', mean((list.input$df_grid$mags - M0)))
    }
    if(model.type == 'depth'){
      eta = theta_[3]*(list.input$df_grid$mags - M0) + th.beta.depth[1]*list.input$df_grid$depth
    }
    if(model.type == 'strike'){
      eta = theta_[3]*(list.input$df_grid$mags - M0) + th.beta.strike[1]*list.input$df_grid$mean.strike
    }
    if(model.type == 'full'){
      eta = theta_[3]*(list.input$df_grid$mags - M0) + th.beta.strike[1]*list.input$df_grid$mean.strike + 
        th.beta.depth[1]*list.input$df_grid$depth
    }
    ...
    # 
    out <- eta + log(theta_[2]) + log(comp.list$It) + 
      log(comp.list$Is)
    cat('Lambda ', sum(exp(out)), '\n')
    return(out)
  }
  ...
  loglambda.inla <- function(th.mu, th.K, th.alpha, th.c, th.p, th.sigma, 
                             th.beta.depth = NULL, th.beta.strike = NULL,
                             bkg, tt, xx, yy, model.type = 'basic',
                             th, xh, yh, mh, depth.h = NULL, mean.strike.h = NULL, 
                             M0, ncore_ = ncore){
    ...
    Sigma. <- matrix(c(sigma.1, 0, 0, sigma.1), byrow = TRUE, ncol = 2)
    if(model.type == 'basic'){
      eta = th.p.df$th.alpha[1]*(mh - M0)  
    }
    if(model.type == 'depth'){
      eta = th.p.df$th.alpha[1]*(mh - M0) + th.beta.depth[1]*depth.h
    }
    if(model.type == 'strike'){
      eta = th.p.df$th.alpha[1]*(mh - M0) + th.beta.strike[1]*mean.strike.h
    }
    if(model.type == 'full'){
      eta = th.p.df$th.alpha[1]*(mh - M0) + th.beta.strike[1]*mean.strike.h + 
        th.beta.depth[1]*depth.h
    }
  ...
  pred.fun <- function(th.mu, th.K, th.alpha, th.c, th.p, th.sigma, 
                       th.beta.depth = NULL, th.beta.strike = NULL,
                       list.input, T1, T2, bdy, M0, model.type = 'basic'){
    ...
    out[list.input$idx.sl] <- loglambda.inla(th.mu = th.mu, 
                                             ...
                                             depth.h = list.input$sample.s$depth,
                                             mean.strike.h = list.input$sample.s$mean.strike,
                                             M0 = M0, model.type = model.type)
    out
  ...
  if(model.type == 'depth'){
    cmp.total <- counts ~ -1 + 
      ...
      th.p(1, model = 'linear', mean.linear = 0, prec.linear = 1) +
      th.sigma(1, model = 'linear', mean.linear = 0, prec.linear = 1) +
      th.beta.depth(1, model = 'linear', mean.linear = 0, prec.linear = 1)
  ...
  if(model.type == 'strike'){
    cmp.total <- counts ~ -1 + 
      ...
      th.p(1, model = 'linear', mean.linear = 0, prec.linear = 1) +
      th.sigma(1, model = 'linear', mean.linear = 0, prec.linear = 1) +
      th.beta.strike(1, model = 'linear', mean.linear = 0, prec.linear = 1)
    form.total <- counts ~ pred.fun(th.mu = th.mu,
                                    th.K = th.K, 
                                    th.alpha = th.alpha,
                                    th.c = th.c, 
...
spatial.ETAS.model.cov <- function(list.input, num.cores = 5, model.type = 'basic'){
  data.bru <- data.frame(x = list.input$catalog.bru.km@coords[,1],
                         y = list.input$catalog.bru.km@coords[,2])
  data.bru <- cbind(data.bru, list.input$catalog.bru.km@data)
  data.bru$mags <- data.bru$magnitudes
  ...
  ETAS.fit.cov(sample.s = data.bru, # data.frame representing data points 
                     ...
                     bru.opt = list.input$bru.opt.list, # verbose changes what inlabru prints 
                     ncore = num.cores, 
               model.type = model.type) # set number of cores
}
...
# 
# lambda.mag.dep <- function(theta, gamma.par, tt, xx, yy, th, xh, yh, mh, M0, list.input){
#   
#   mh.round <- list.input$mh.round#round(mh,1)
#   mh.unique <- list.input$mh.unique #unique(mh.round)
#   mh.mapping <- list.input$mh.mapping #match(mh.round, mh.unique)
#   Sigma.list <- lapply(mh.unique, function(x) matrix(c(exp(gamma.par*(x - M0 + 1)), 0,
#                                                        0, exp(gamma.par*(x - M0 + 1))),
#                                                      byrow = TRUE, ncol = 2))
#   sum.gx <- 0
#   for(mh.idx in 1:length(mh.unique)){
#     mh.value <- mh.unique[mh.idx]
#     Sigma.mh <- Sigma.list[[mh.idx]]
#     idx.sel <- which(mh.mapping == mh.idx)
#     th.mh <- th[idx.sel]
#     xh.mh <- xh[idx.sel]
#     yh.mh <- yh[idx.sel]
#     mh.mh <- mh[idx.sel]
#     sum.gx <- sum.gx + sum(g.x(theta = theta, tt = tt, xx = xx, yy = yy,
#                                th = th.mh, xh = xh.mh,
#                                yh = yh.mh, mh = mh.mh,
#                                M0 = M0, Sigma = Sigma.mh))
#   }
#   theta[1] + sum.gx
# }
