...
space.time.grid <- function(data.point, delta.sp, n.layer., min.edge., 
                            ...
                            T2., bdy., mag.dep = FALSE,
                            isotropic = TRUE,
                            displaygrid = FALSE){
  xx. <- data.point$x
  yy. <- data.point$y
  tt. <- data.point$ts
  mm. <- round(data.point$mags,0)
  ...
  space.bins <- foreach(l.name = layer.name, .combine = rbind) %do% {
    ...
    df.out <- data.frame(x.min = min(df.sel$x),
                         y.min = min(df.sel$y),
                         x.max = max(df.sel$x),
                         y.max = max(df.sel$y),
                         bin.name = paste0(l.name,'_',idx.p))
    if(mag.dep){
      df.out$bin.name <- paste0(df.out$bin.name, '_', mm.)
      df.out
    } else{
      df.out
    }
  ...
  if(!mag.dep & isotropic){
    spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer)] <- str_extract(spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer)], "^\\d+")
    return(spt.grid)  
  }
  if(mag.dep & isotropic){
    first.part <- str_extract(spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer)], '^\\d+')
    last.part <- str_extract(spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer)], "_[^_]+$")
    spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer)] <- paste0(first.part, last.part)
    return(spt.grid)
  }
  if(mag.dep & !isotropic){
    return(spt.grid)
  ...
  if(!mag.dep & !isotropic){
    idx.mm.sel <- mm. >= 5.5
    first.part.under <- str_extract(spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer) & !idx.mm.sel], '^\\d+')
    spt.grid$ref_layer[!grepl('last', spt.grid$ref_layer) & !idx.mm.sel] <- paste0(first.part.under,'.under')
    first.part.over <- sub('_[0-9]+\\.[0-9]+$', '', spt.grid$ref_layer[idx.mm.sel])
    spt.grid$ref_layer[idx.mm.sel] <- paste0(first.part.over, '.over')
    return(spt.grid)
  }
}
...
compute.grid <- function(param., list.input){
  ...
  if(is.list(list.input$Sigma_)){
    Sigma.sp <- list.input$Sigma_[sapply(list.input$sp.names, \(bname) which(list.input$gridd$ref_layer == bname)[1])]
    cat('Sigma.sp length', length(Sigma.sp), '\n')
...
ETAS.fit.spatial <- function(sample.s, 
  ...
  df.j <- foreach(idx = 1:nrow(sample.s), .combine = rbind) %do% {
    space.time.grid(data.point = sample.s[idx,], 
                    ...
                    N.max = N_max,
                    T2. = T2,
                    bdy. = bdy, mag.dep = mag.dep, isotropic = isotropic, 
                    displaygrid = FALSE)
  } 
  df.j$counts = 0
  df.j$exposures = 1
  ...
  logLambda.h.inla <- function(th.K, th.alpha, th.c, th.p, th.sigma1, 
    ...
    if(mag.dep & isotropic){
      gamma.p <- link.functions$sigma(th.sigma1[1])
      cat('gamma.p = ', gamma.p, '\n')
      rounded.mags <- round(list.input$df_grid$mags, 1)
      Sigma. <- lapply(1:nrow(list.input$df_grid), function(x)
        matrix(c(exp(gamma.p*(rounded.mags[x] - M0 + 1)), 0,
                 0, exp(gamma.p*(rounded.mags[x] - M0 + 1))),
               byrow = TRUE, ncol = 2))
    }
    ...
    # 
    out <- theta_[3]*(list.input$df_grid$mags - M0) + log(theta_[2]) + log(comp.list$It) + 
      log(comp.list$Is)
    cat('logLambda =', sum(exp(out)), '\n')
    out
  }
  ...
  loglambda.inla <- function(th.mu, th.K, th.alpha, th.c, th.p, th.sigma1, 
                             bkg, tt, xx, yy, 
                             th, xh, yh, mh, M0, ncore_ = ncore, list.input, 
                             mag.dep = FALSE, isotropic = TRUE){
    ...
    } else if(mag.dep & isotropic){
      gamma.p <- link.functions$sigma(th.sigma1[1])
      #cat('gamma.p in loglambda = ', gamma.p, '\n')
      outp <- unlist(mclapply(1:length(tt), \(idx) 
                              log(lambda.mag.dep(
                                theta = as.numeric(th.p.df[idx,]), 
                                gamma.par = gamma.p,
                                tt = tt[idx], 
                                xx = xx[idx], 
                                yy = yy[idx], 
  ...
  mh.round <- round(sample.s$mags, 0)
  mh.unique <- unique(mh.round)
  mh.mapping <- match(mh.round, mh.unique)
  list.input. <- list(t.names = t.names, 
                      ...
                      mh.round = mh.round,
                      mh.unique = mh.unique, 
                      mh.mapping = mh.mapping)
  
  pred.fun <- function(th.mu, th.K, th.alpha, th.c, th.p, th.sigma1,
                       list.input, T1, T2, bdy, M0, mag.dep = FALSE, isotropic = TRUE){
    ...
    out[list.input$idx.trig] <- logLambda.h.inla(th.K = th.K, 
                                                 ...
                                                 list.input = list.input,
                                                 M0 = M0, bdy = bdy,
                                                 mag.dep = mag.dep, isotropic = isotropic 
                                                )
...
spatial.ETAS.model <- function(list.input, mag.dep = FALSE, isotropic = TRUE,
                               num.cores = 5){
  data.bru <- data.frame(x = list.input$catalog.bru.km@coords[,1],
                         y = list.input$catalog.bru.km@coords[,2],
                         mags = list.input$catalog.bru.km$magnitudes,
  ...
  ETAS.fit.spatial(sample.s = data.bru, # data.frame representing data points 
                     ...
                     ncore = num.cores,
                   mag.dep = mag.dep, 
                   isotropic = isotropic) # set number of cores
}
